...
def run_func(op_code_node):
    ...
    # table을 보고 함수를 작성하시오
    def plus(node):
        operand1 = strip_quote(node)
        operand2 = strip_quote(node).next
        if operand1.type is TokenType.LIST:
            operand1 = run_expr(operand1)
        if operand2.type is TokenType.LIST:
            operand2 = run_expr(operand2)
        return Node(TokenType.INT, int(operand1.value)+int(operand2.value))
    ...
    def lt(node):
        ...
        if operand2.type is TokenType.LIST:
            operand2 = run_expr(operand2)
        if int(operand1.value) < int(operand2.value):
            return Node(TokenType.TRUE)
        else:
            return Node(TokenType.FALSE)
    ...
    def not_op(node):
        operand = strip_quote(node)
        if operand.type is TokenType.FALSE:
            return Node(TokenType.TRUE)
        else:
            return Node(TokenType.FALSE)
    ...
    def run_cond(node):
        ...
        """
        #Fill Out
        if node.value.type is CuteType.TRUE or node.value.type is CuteType.FALSE:
            condition = node.value
        else:
            condition = run_expr(node.value)
        result = node.value.next.value
